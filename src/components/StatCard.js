...
export default function StatCard({ titulo, valor, sub, color = '#F4821F', icono, alerta, onClick }) {
  return (
    <div
      style={{ ...s.card, borderTop: `3px solid ${color}`, ...(onClick ? s.cardClickable : {}) }}
      onClick={onClick}
      title={onClick ? `Ver ${titulo}` : undefined}
    >
      <div style={s.row}>
      ...
      {onClick && <div style={s.verDetalle}>Ver detalle →</div>}
    </div>
  );
}
...
const s = {
  ...
  verDetalle: { fontSize: 11, color: '#F4821F', marginTop: 8, fontWeight: 600 },
};
